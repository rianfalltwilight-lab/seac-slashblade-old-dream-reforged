...
public final class DualModeClientProbe {
    ...
    private static final List<Map<String,Object>> trailFrames=new ArrayList<>(),motionEvents=new ArrayList<>();
    ...
    @SubscribeEvent public static void tick(ClientTickEvent.Post event){
        ...
        try {
            ...
            else if(stage==10){check(frames.stream().anyMatch(r->r.get("mode").equals(1)),"No native renderer frames");check(frames.stream().anyMatch(r->r.get("mode").equals(2)),"No restored old renderer frames");write("checks.json",rows);write("frames.json",frames);write("trail-frames.json",trailFrames);write("motion-events.json",motionEvents);write("result.json",Map.of("passed",failures.isEmpty(),"failures",failures,"captures",captures,"renderer",com.mojang.blaze3d.platform.GlUtil.getRenderer(),"scope","GUI key request and server ack, two-way modes, combo renders in two views, single projectiles, dimension and death; integrated single client"));done=true;mc.stop();}
        }catch(Throwable e){done=true;try{write("frames.json",frames);write("trail-frames.json",trailFrames);write("motion-events.json",motionEvents);write("failure.json",Map.of("error",e.toString(),"stage",stage,"tick",tick,"mode",mode,"checks",rows,"failures",failures));}catch(Exception ignored){}com.mojang.logging.LogUtils.getLogger().error("DUAL_MODE_CLIENT_FAILED",e);mc.stop();}
    }
    ...
    @SubscribeEvent public static void trailFrame(mods.flammpfeil.slashblade.event.client.RenderOverrideEvent e){
        if(!enabled() || done || stage!=3 || mode==1 || !Set.of("blade","obj1").contains(e.getTarget()))return;
        var mc=Minecraft.getInstance();var p=mc.player;var s=BladeStateAccess.of(p.getMainHandItem()).orElseThrow();var combo=s.peekCurrentComboStateTicks(p).getValue();
        var row=new LinkedHashMap<String,Object>();row.put("frame",renderedFrame);row.put("mode",mode);row.put("camera",camera);row.put("click",click);row.put("input_tick",tick);row.put("world_tick",p.level().getGameTime());row.put("part",e.getTarget());row.put("hand",e.getStack()==p.getMainHandItem()?"main":e.getStack()==p.getOffhandItem()?"off":"other");row.put("combo",combo.toString());row.put("action_time",s.getLastActionTime());row.put("swing_time",p.swingTime);row.put("swinging",p.swinging);row.put("attack_anim",p.attackAnim);row.put("old_attack_anim",p.oAttackAnim);row.put("partial",mc.getTimer().getGameTimeDeltaPartialTick(false));row.put("pose",e.getPoseStack().last().pose().get(new float[16]));trailFrames.add(row);
    }
    @SubscribeEvent public static void motion(mods.flammpfeil.slashblade.event.BladeMotionEvent e){if(enabled() && !done && e.getEntity().level().isClientSide && e.getEntity()==Minecraft.getInstance().player)motionEvents.add(Map.of("frame",renderedFrame,"mode",mode,"camera",camera,"click",click,"stage",stage,"combo",e.getCombo().toString(),"action_time",e.getActionTime(),"world_tick",e.getEntity().level().getGameTime()));}
    @SubscribeEvent public static void frame(RenderFrameEvent.Post e){renderedFrame++;if(!enabled())return;if(out!=null && (done || stage==10))try{write("trail-frames.json",trailFrames);write("motion-events.json",motionEvents);}catch(Exception ex){failure=ex;}if(done || capture==null)return;try(var image=Screenshot.takeScreenshot(Minecraft.getInstance().getMainRenderTarget())){image.writeToFile(out.resolve(capture));captures.add(capture);capture=null;}catch(Exception ex){failure=ex;}}
}
