...
public final class LegacyViewProbe {
    private static final String[] BLADES={"slashblade:sange","slashblade_addon:kamuy_lightning"};
    ...
    @SubscribeEvent public static void tick(ClientTickEvent.Post event){
        ...
        try {
            ...
            if(phase==0 && mc.screen instanceof TitleScreen && mc.getOverlay()==null){capture="01-title.png";phase=1;}
                ...
                if(ticks>=24){ticks=0;if(++blade==BLADES.length)phase=7;else {phase=4;prepared=false;mc.options.setCameraType(CameraType.FIRST_PERSON);prepare(mc);}}
            }else if(phase==7){
                ...
                if(!missing.isEmpty() || matrices.isEmpty() || captures.size()!=21)throw new IllegalStateException("Incomplete rendered views "+captures.size()+" "+missing);
    ...
    @SubscribeEvent public static void bladeFrame(mods.flammpfeil.slashblade.event.client.RenderOverrideEvent event){
        if(!Boolean.getBoolean("scex.legacy.viewProbe") || phase!=5 || done)return;
        var mc=Minecraft.getInstance();
        if(event.getStack()!=mc.player.getMainHandItem() || !Set.of("blade","sheath").contains(event.getTarget()))return;
        matrices.add(Map.of("blade",blade,"view",view,"tick",ticks,"target",event.getTarget(),"yaw",mc.player.getYRot(),"pitch",mc.player.getXRot(),
            "pose",event.getPoseStack().last().pose().get(new float[16]),"modelView",com.mojang.blaze3d.systems.RenderSystem.getModelViewMatrix().get(new float[16])));
    }
